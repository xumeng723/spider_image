...
# 根据svid下载图片
def grab_img(svid, out_dir,num,cate,long,lati):
...
def main(LL,output_dir,num,cate,long,lati):
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    MER = LL2MER(LL[0], LL[1])
    svid = get_svid(MER)
    grab_img(svid, output_dir,num,cate,long,lati)

if __name__ == "__main__":
    fp = open("E:\\didi\\merge\\1.txt", "r")
    ...
    for line in fp.readlines():
        ...
        main(LL,output_dir,num,cate,long,lati)
    #output_dir = 'output_image'
    #main(LL,output_dir)